# 10-wawon-female — わ行 (row 10), the series FINALE · Female / city-pop cut
...
**This is the female / city-pop cut.** The finale is written once and rendered in **all three
house voices** — Strategy B (`../../personas.md`): `10-wawon-female` (this), `10-wawon-male`
(koto), `10-wawon-kids` (graduation singalong). Same lyric, three capstones, one per playlist.
...
Generate in Suno from `suno-v3.txt` (Persona: Female JP) → pick take → `audio/master.wav`,
fetch `il/` → `build_learning_timing 10-wawon-female` → align → `gen_registry` → `sync` →
render → `gen_description`. Then re-voice for `-male` and `-kids`.
